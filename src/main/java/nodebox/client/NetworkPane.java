...
public class NetworkPane extends Pane {
    ...
    public NetworkPane(NodeBoxDocument document) {
        ...
        document.getDocumentFrame().addZoomListener(networkView);
        networkView.setAlignmentX(JComponent.LEFT_ALIGNMENT);
        //add(networkView);
        
        //if the document is in the first row, put the paneHeader on top
        //if (document.getIsFirstRow()){
	        //add(paneHeader);
	        add(errorLabel);
	        add(networkView);
        //}
        /*else{ //if it's in the second row put networkView on top
	        add(networkView);
	        //add(paneHeader);
	        add(errorLabel);
        }*/
