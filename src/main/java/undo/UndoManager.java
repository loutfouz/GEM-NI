...
import nodebox.client.NodeLibraryUndoableEdit;
...
public class UndoManager extends CompoundEdit implements UndoableEditListener {
    ...
    /**
     * Copy constructor, using a static newInstance
     * method. Added by Shumon November 22, 2013
     */
     public static UndoManager newInstance(NodeBoxDocument oldDoc, NodeBoxDocument newDoc) {
    	 UndoManager oldUndoManager = oldDoc.undoManager;
    	 UndoManager newUndoManager = new UndoManager();

    	 newUndoManager.alive = oldUndoManager.alive;
    	 newUndoManager.hasBeenDone = oldUndoManager.hasBeenDone;
    	 newUndoManager.indexOfNextAdd = oldUndoManager.indexOfNextAdd;
    	 newUndoManager.inProgress = oldUndoManager.inProgress;
    	 //newUndoManager.limit = oldUndoManager.limit;
    	 
    	 newUndoManager.edits = (Vector<UndoableEdit>) oldUndoManager.edits.clone();
    	 
    	 //we will ignore active network for now 
    	 //nodelibrary needs to be replaced by the nodelibrary of the newly created document
    	 for (int i = 0; i < newUndoManager.edits.size(); i++){
    		 NodeLibraryUndoableEdit edit = (NodeLibraryUndoableEdit) oldUndoManager.edits.get(i);
    		 
    		 NodeLibraryUndoableEdit newEdit = new NodeLibraryUndoableEdit(edit.undoState.nodeLibrary, newDoc, edit.command, edit.undoState.activeNetworkPath, edit.undoState.activeNodeName);
    		 //edit.setNodeLibrary(newDoc.getNodeLibrary());
    		 //edit.document = newDoc;
    		 /*if (newEdit.undoState != null)
    			 newEdit.undoState.nodeLibrary = newDoc.getNodeLibrary();
    		 if (newEdit.redoState != null)
    			 newEdit.redoState.nodeLibrary = newDoc.getNodeLibrary();
    		 */
    		 newUndoManager.edits.set(i, newEdit); //replace edit with the new edit
    	}
...
//          System.out.println("new index " + indexOfNextAdd);
    ...
    /**
     * Returns a string list with all edits.
     * 
     * @return a string list representing all edits
     */
    public List<String> getEditStringList() {
    	List<String> editsList = new ArrayList<String>();
    	for (UndoableEdit edit : edits)
    		editsList.add(edit.getPresentationName());
    	return editsList;
    }
}
